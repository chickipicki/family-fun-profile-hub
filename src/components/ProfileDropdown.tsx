...
const ProfileDropdown = ({
  ...
  return <div className="relative">
      {isHighlighted && !hasSeenTooltip && <TooltipProvider>
          <Tooltip open={true}>
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-56">
          ...
          {profiles.map(profile => <DropdownMenuItem key={profile.id} className={`flex items-center space-x-2 cursor-pointer ${activeProfile.id === profile.id ? 'border-2 border-primary bg-secondary' : ''}`} onClick={() => onProfileChange(profile)}>
      ...
      <CreateChildProfileForm isOpen={isAddProfileOpen} onClose={() => setIsAddProfileOpen(false)} onCreateProfile={onProfileChange} />
    </div>;
};
